...
- **Orbita's own state lives in `~/.orbita/`** (distinct from the agent dotdirs it manages): `CapabilitySnapshotStore` persists scan snapshots (`CapabilitySnapshot`, schema-versioned), and `ProjectLibraryStore` persists the recent-projects list. Bump the schema version when changing those shapes **or when a scan/resolve change alters what the persisted graph contains** (e.g. a new agent's discovery rules) — otherwise a stale snapshot is served as a fresh cache hit. An unreadable/incompatible state file is moved aside non-destructively by the shared `OrbitaStateBackup` (first to `<file>.bak`, then `.bak.1`, … — it never clobbers an existing backup or pre-deletes one).
