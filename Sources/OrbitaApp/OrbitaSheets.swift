...
struct ManageAgentsSheet: View {
    let defaultOrder: [AgentSelection]
    let onSave: ([AgentSelection]) -> Void
    let onCancel: () -> Void

    @State private var orderedAgents: [AgentSelection]

    init(
        agentOptions: [AgentSelection],
        defaultOrder: [AgentSelection],
        onSave: @escaping ([AgentSelection]) -> Void,
        onCancel: @escaping () -> Void
    ) {
        self.defaultOrder = defaultOrder
        self.onSave = onSave
        self.onCancel = onCancel
        _orderedAgents = State(initialValue: agentOptions)
    }
                ...
                VStack(spacing: 8) {
                    ForEach(Array(orderedAgents.enumerated()), id: \.element.id) { index, agent in
                        ManageAgentOrderRow(
                            agent: agent,
                            index: index,
                            isFirst: index == orderedAgents.startIndex,
                            isLast: index == orderedAgents.index(before: orderedAgents.endIndex),
                            onMoveUp: {
                                moveAgent(from: index, to: index - 1)
                            },
                            onMoveDown: {
                                moveAgent(from: index, to: index + 1)
                            }
    ...
    private func moveAgent(from sourceIndex: Int, to destinationIndex: Int) {
        guard orderedAgents.indices.contains(sourceIndex),
              orderedAgents.indices.contains(destinationIndex),
              sourceIndex != destinationIndex else {
            return
        }
        withAnimation(.snappy(duration: 0.18)) {
            let agent = orderedAgents.remove(at: sourceIndex)
            orderedAgents.insert(agent, at: destinationIndex)
        }
    }
}

private struct ManageAgentsHeader: View {
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 14, style: .continuous)
...
private struct ManageAgentOrderRow: View {
    ...
    private var agentKindLabel: String {
        if agent.id.hasPrefix("built-in:") {
            return "Built-in"
        }
        if agent.behavior == .skillsAgent {
            return "Skills CLI"
        }
        return "Custom"
    }
...
private struct ManageAgentMoveButton: View {
    let systemImage: String
    let isDisabled: Bool
    let action: () -> Void
